...
const mysql = require('./../@thirdday/db-mysql');
const oAuth = require('./../@thirdday/oAuth');
const EventEmitter = require('events');

class MyEmitter extends EventEmitter {};
global.G_EVENTS = new MyEmitter();    // 全局事件
...
redis.start();
// Redis加载完成
G_EVENTS.on("Redis_Ready", function() {
    console.log("Redis客户端加载完成");

    mysql.start(router, {});
    // Mysql加载完成
    G_EVENTS.on("DB_Mysql_Ready", function() {
        console.log("Mysql数据库客户端加载完成");
